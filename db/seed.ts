...
async function seed() {
  ...
  // Course 1: JavaScript cơ bản
  const [course1] = await db.insert(courses).values({
    title: 'JavaScript Cơ Bản',
    titleEn: 'JavaScript Basics',
    description: 'Học lập trình JavaScript từ zero đến hero. Khóa học phù hợp cho người mới bắt đầu.',
    descriptionEn: 'Learn JavaScript programming from zero to hero. Suitable for beginners.',
  ...
  const [lesson1] = await db.insert(lessons).values({
    ...
    content: '## JavaScript là gì?\n\nJavaScript là ngôn ngữ lập trình phổ biến nhất thế giới, chạy trên trình duyệt và máy chủ.',
  ...
  await db.insert(quizzes).values({
    lessonId: lesson1.id,
    questions: [
      { q: 'JavaScript chạy ở đâu?', options: ['Chỉ server', 'Chỉ browser', 'Cả browser và server', 'Không nơi nào'], correctIndex: 2, explanation: 'JavaScript chạy cả trên browser (V8, SpiderMonkey) và server (Node.js).' },
  ...
  const [course2] = await db.insert(courses).values({
    ...
    descriptionEn: 'Build modern web applications with React, hooks, and state management.',
