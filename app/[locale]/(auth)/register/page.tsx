      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="name" className="text-sm text-slate-300">{t('name')}</Label>
            <Input
              id="name"
          ...
          <Button
            type="submit"
            disabled={loading}
            className="mt-2 h-10 w-full bg-indigo-600 font-semibold text-white hover:bg-indigo-500"
          >
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          {t('hasAccount')}{' '}
          <Link href={`/${locale}/login`} className="font-medium text-indigo-400 hover:text-indigo-300">
            {t('login')}
          </Link>
        </p>
      </div>
